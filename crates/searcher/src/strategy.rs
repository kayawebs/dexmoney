...
use base_arb_dex::balancer_v3::quote_weighted_exact_in as quote_balancer_weighted_exact_in;
            ...
            PoolVariant::BalancerV3 => {
                if pool_state.balancer_model.as_deref() == Some("weighted") {
                    ...
                    let router = balancer_v3_router.ok_or_else(|| {
                        QuoteSkip::quote_error(
                            "BALANCER_V3_ROUTER is required for Balancer V3 quote",
                        )
                    })?;
...
mod tests {
    ...
    #[test]
    fn quotes_balancer_weighted_two_token_pool_locally() {
        let usdc = address!("833589fCD6eDb6E08f4c7C32D4f71b54bdA02913");
        let weth = address!("4200000000000000000000000000000000000006");
        ...
        let out = quote_balancer_weighted_exact_in(&state, usdc, weth, U256::from(5_000_000u64))
